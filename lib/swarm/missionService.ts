...
/**
 * Same query as listMissions(), but throws on a real query failure instead
 * of silently returning []. M4-06: listMissions()'s "swallow and return []"
 * contract is relied on by several existing callers (dashboardService,
 * healthService, use-gbrain-data) that treat "no missions" and "query
 * failed" the same way on purpose — this variant exists for the one caller
 * that needs to tell them apart (stores/dashboardStore.ts's loadMissions(),
 * which surfaces a real missionsError instead of silently leaving stale
 * state with zero signal).
 */
export async function listMissionsOrThrow(
  limit = 50,
  status?: MissionStatus,
  tenantId?: string,
): Promise<Mission[]> {
