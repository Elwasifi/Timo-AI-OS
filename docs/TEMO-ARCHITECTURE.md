...
Version: 3.5
Date: 2026-08-26
Status: M1-01 through M1-08 (Milestone 1 — Reliability & Safety) implemented and live-verified where applicable. M1-01: the tool executor is wired into the mission engine's `executeTask()`. M1-02: `checkBudget()` is a real hard gate inside `chatWithFallback()`/`streamWithFallback()`. M1-03: a Postgres token-bucket rate-limits the 2 API routes that can trigger an AI call or tool execution. M1-04: n8n's `handleRun()` now polls real execution history for a genuine terminal status instead of trusting the trigger's HTTP code. M1-05: the chat page resumes the tenant's most recent conversation on load. M1-06: audited all 23 `app/api/**` routes, fixed 2 real IDOR gaps, flagged 16 globally-scoped routes as an open product decision. M1-07: Vitest added (`npm test`) with 3 real integration test files, all passing against the live database. M1-08: `docs/runbooks/local-n8n-dev-setup.md` written from directly-inspected real infrastructure (Docker container config, the live `cloudflared` process), not assumption; the quick-tunnel-vs-named-tunnel question flagged as Amro's decision, not resolved. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
