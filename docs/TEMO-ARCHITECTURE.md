...
Version: 3.6
Date: 2026-08-26
Status: **Milestone 1 (Reliability & Safety) complete — M1-01 through M1-09, all implemented and live-verified.** M1-01: the tool executor is wired into the mission engine. M1-02: a real budget hard-gate inside `chatWithFallback()`/`streamWithFallback()`. M1-03: Postgres token-bucket rate limiting on the 2 routes that can trigger AI/tool spend. M1-04: n8n execution success is now verified against real execution history, not the trigger's HTTP code. M1-05: chat conversations persist across a refresh. M1-06: all 23 API routes audited, 2 real IDOR gaps fixed, 16 globally-scoped routes flagged as an open product decision. M1-07: Vitest with 3 real integration tests (tenant isolation, auth gate, mission lifecycle), all passing live. M1-08: the local n8n/cloudflared dev setup is now a written runbook, not tribal knowledge. M1-09: Internal Operator Mode's first capability (create an n8n workflow from a description) is live, approval-gated, and proven structurally impossible to execute for a client tenant even after a mistaken approval — a template for every future operator capability. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
