...
Version: 3.11
Date: 2026-08-26
Status: Milestone 1 and Milestone 2 complete and merged to `main`. Milestone 3 (Speed & Stability + Visual/UX fixes) underway on `milestone-3-experience`: **M3-01, M3-02, M3-04, and M3-05 all done and live-verified.** M3-01 fixed two dead/misconfigured provider models and the real silent-non-response bug (reasoning models returning empty content on a 200 when maxTokens is exhausted by internal reasoning), added a health-based skip-before-retry, and bounded the intent classifier's hot-path exposure. M3-02 added a real typing indicator and an immediate mission acknowledgment so the user is never looking at silence while a response is being generated. M3-04 renamed Command Deck to Main Dashboard and gave the hero-bridge mini team view a real link to the full G-Brain page. M3-05 fixed the chat-page mic's navigate-to-Settings bug and replaced Main Dashboard's icon-only, unlabeled voice popover with a single clean mic trigger under Temo that expands into a recording-bar. M3-03 (G-Brain radial layout redesign) and M3-06 (Timo persona/system-prompt pass) remain explicitly out of scope for this milestone, queued separately. Predecessor milestones: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
