...
Version: 3.2
Date: 2026-08-26
Status: M1-01 through M1-05 (Milestone 1 — Reliability & Safety) implemented and live-verified. M1-01: the tool executor is wired into the mission engine's `executeTask()`. M1-02: `checkBudget()` is a real hard gate inside `chatWithFallback()`/`streamWithFallback()`. M1-03: a Postgres token-bucket rate-limits the 2 API routes that can trigger an AI call or tool execution. M1-04: n8n's `handleRun()` now polls real execution history for a genuine terminal status instead of trusting the trigger's HTTP code, plus two more real n8n-related bugs found and fixed. M1-05: the chat page now resumes the tenant's most recent conversation on load (localStorage first, DB fallback) instead of always starting fresh — `ConversationService` was already persisting every message for real, the gap was purely the client-side load lifecycle; live-verified with a real page reload and a fresh-browser-context DB-fallback test. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
