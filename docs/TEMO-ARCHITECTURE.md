...
Version: 2.8
Date: 2026-08-26
Status: M1-01 (Milestone 1 — Reliability & Safety) implemented and fully live-verified, including the real n8n success path: the tool executor is now wired into the mission engine's `executeTask()`, reusing the chat path's `decideTools()` rather than duplicating it, so mission tasks can genuinely call tools (n8n, memory, etc.) with `requiresApproval`/`isSimulation` respected exactly as they already are for chat. Two classes of latent bugs were found and fixed in the same pass: (1) the tool registry was only ever populated client-side, so the background task-queue processor (a server process) would have silently never executed any tool a mission task asked for — fixed process-agnostically; (2) once local n8n + `cloudflared` became available, three real bugs in `supabase/functions/n8n-proxy` surfaced and were fixed (missing required `settings` on workflow creation, wrong response-unwrapping on every single-resource n8n endpoint, `convertToWebhook` never activating the workflow it created) — none of this code had ever round-tripped through a live n8n instance before. Live-verified end to end: a real tool success (task completes, LLM skipped), a real tool failure (task fails after retries with a genuine error, not swallowed), a real `requiresApproval` gate (blocked, linked to the originating mission/task in `approval_requests`), and a real n8n workflow trigger confirmed via n8n's own execution history (real execution row, `status: "success"`). Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback`, deciding provider+model per request from task classification, real provider-reported capability/pricing data, and live health tracking — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
