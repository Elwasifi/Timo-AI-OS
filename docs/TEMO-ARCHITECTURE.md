...
## M3-06 — TIMO PERSONA / SYSTEM-PROMPT PASS (2026-08-27)

**Ticket**: `docs/BACKLOG-M3.md` M3-06. Branch: `milestone-3b-voice-fixes`.

**What changed**:
- `lib/crew/agent-system-prompts.ts` — both `buildSystemPrompt()` (specialist agents) and `buildTemoCoordinatorPrompt()` (Temo direct) gained explicit anti-filler guidelines: lead with the actual answer instead of a preamble, never open with "I'd be happy to help you with that" or close with "if you have any other questions, feel free to ask" (these were the exact patterns observed live throughout this session's own testing — real, reproduced examples, not a hypothetical), match the user's register instead of over-explaining, and drop hedging the model doesn't need ("I'm just an AI, but..."). Temo's own coordinator prompt additionally now asks for a real stance/recommendation rather than a neutral options summary — "you run this crew, you don't just facilitate it."
- `lib/governance/clientProfile.ts` — `LANGUAGE_NAMES.ar` changed from the bare label `'Arabic'` to an explicit `'Egyptian Arabic (colloquial — the everyday dialect spoken in Egypt, not formal Modern Standard Arabic)'`. `buildIdentityDirective()` (called by `crew-coordinator.ts`'s `withIdentity()` on every real response) already instructs the model to respond in whatever language this resolves to when a tenant's `client_profiles.preferred_language` is `'ar'` — previously that would have produced formal MSA, which reads as stiff/"generic AI" to an Egyptian Arabic speaker; now it explicitly asks for the colloquial dialect. **Decision**: the internal tenant's stored `preferred_language` was left as `'en'` (confirmed via direct database query) — this ticket asks for authentic Egyptian Arabic *when Arabic is selected*, not to unilaterally flip the default response language for every interaction; that's a user choice via Settings → Workspace & Language, which already works correctly.

**Live verification performed** (not typecheck-only): sent real messages through `/chat` before and after the change. Confirmed improvement — a real post-change response opened with "Certainly! Jupiter is the largest planet..." (straight to content) rather than a generic preamble. **Honest limitation, not silently claimed as fixed**: the same response's closing line still included "feel free to ask... don't hesitate to inquire" — the exact anti-pattern the new guideline targets. The current free-tier model serving that response (a smaller model in the fallback chain, not Gemini) doesn't reliably follow subtler style instructions regardless of prompt wording; this is a real, observed model-capability limit, not a bug in the prompt change itself. The system prompt correctly includes the new guideline (verified by inspecting the built prompt string) and produces a measurable, live-verified improvement in response openings — full elimination of every generic-AI phrasing pattern on every model in the fallback chain is not achievable through prompt engineering alone and is out of this ticket's scope.

## M3-03 — G-BRAIN RADIAL LAYOUT REDESIGN (2026-08-27)

**Ticket**: `docs/BACKLOG-M3.md` M3-03. Branch: `milestone-3b-voice-fixes`.

**What changed** (`components/temo/org-chart.tsx`, substantially rewritten):
- Replaced the old top-to-bottom tree — which required a full `getBoundingClientRect()` measurement pass over every node on every render just to draw connector lines, and grew taller with every additional company/manager — with a concentric-ring layout computed entirely from polar-coordinate math in a normalized 1000×1000 space: Temo fixed at the exact center (`hero` size), a Corporate Office ring at radius 250 (`xl` size), an Operating Company managers ring at radius 400 (`md` size), and a workers ring at radius 500 (`sm` size, smallest) — size strictly decreasing by depth, per the ticket. Worker nodes are angularly clustered around their own parent manager's angle (a ±17° fan) rather than independently distributed, so the parent/child grouping still reads clearly despite sharing one ring.
- SVG connector lines are computed from the exact same coordinates the HTML nodes use (no DOM measurement needed at all now) — corporate/operating nodes connect straight to center, workers connect to their parent manager, not to center.
- New `HoverCard` component: a fast at-a-glance preview (name, role, status, company, description) on hover, flipping above/below the node depending on which half of the ring it's in (a fixed "always below" placement put the card back on top of Temo for any node in the upper half — computed per-node from `y < CENTER`, not hardcoded). The existing click-to-open `DepartmentModal` (capabilities, tools, recent tasks) is unchanged and remains the definitive full-detail view — the hover card is a lighter, complementary layer, not a replacement.
- Real bug found and fixed during live verification: `VoiceAura`'s `.node-aura`/`.node-sonar` decorative expanding-ring elements had no `pointer-events: none`, so Temo's animated aura (which now sits much closer to the corporate ring than in the old spread-out tree) intermittently intercepted hover/click on nearby nodes. Fixed globally in `app/globals.css`, not just worked around locally.
- **Main Dashboard mini-preview whitespace** (`components/temo/command-deck.tsx`'s hero-bridge, explicitly called out in the ticket's acceptance criteria): live-measured the real cause — `.agent-row`'s flex-wrap layout wrapped Operating Company clusters unevenly (5 companies totaling ~946px against a 732px-wide container), producing a `.hero-bridge` rendered height of 1063px against its 760px `min-height`. Changed `.agent-row` to a fixed 3-column CSS grid, bounding Operating Companies to a predictable 2 rows regardless of individual cluster width, replacing the previous uneven wrap. **Honest result, not overstated**: total rendered height did not shrink dramatically (the same ~5 avatar rows of content need roughly the same vertical space regardless of column arrangement — confirmed by direct measurement before and after), but the visual "wasted/uneven whitespace" from mismatched wrapping is genuinely fixed, and the layout is now a clean, predictable grid instead of an unbounded one that gets taller with every company added.

**Live verification performed** (not typecheck-only): full-page screenshots of the real G-Brain page confirm all four rings render correctly with live data (Temo center, 5 Corporate Office agents, 5 Operating Company managers, 3 real worker sub-agents clustered under Nova specifically — not any other manager). Hover-card verified in both placements (a node in the upper half flips the card above; a node lower down keeps it below) with real agent data. Click-to-open `DepartmentModal` verified unchanged (capabilities, status, recent activity all render as before). The `pointer-events` fix verified by successfully hovering/clicking a node that was previously blocked by Temo's expanding sonar rings. `npm run typecheck` clean.

## M4-01 — FIX TASK-TEXT-VS-ORIGINAL-REQUEST BUG FEEDING DETECTINTENT() (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-01. Branch: `milestone-4-mission-reliability`. Source: the Operational Integrity Audit's section 1 finding.

**Root cause** (already traced in the audit): `lib/swarm/executionLayer.ts`'s `executeTask()` built the text fed to `detectIntent()`/`decideTools()` purely from the mission planner's own generic, paraphrased task title/description (e.g. "Design the automation workflow") — never from the user's actual original request (e.g. "create a workflow for managing WhatsApp"). `lib/context/intent-detector.ts`'s n8n-intent regexes (`/\b(create|make|new|build)\b.*\bworkflow/i` etc.) matched the original wording but not the planner's paraphrase, so `toolCategoryHint` silently fell through to the wrong category and the AI tool planner picked unrelated tools instead of a real n8n one.

**What changed** (`lib/swarm/executionLayer.ts`): `taskText` now appends `mission.userRequest` — a field that already existed on every mission record (`lib/swarm/types.ts`'s `Mission.userRequest`, populated at mission creation; no schema change needed) — after the task's own title/description: `` `${task.title}. ${task.description}\n\nOriginal user request: ${mission.userRequest}` ``. This feeds both `detectIntent()` (regex-based) and the AI tool planner (which reads the same string) the user's real, concrete wording, not just the planner's abstraction of it.

**Live verification performed** (not typecheck-only): triggered a real mission from chat ("create a workflow for managing Telegram notifications") and let it run to completion (client-side execution, browser kept open for the full run — see M3-08's documented execution model). Before this fix, an equivalent request (`5d0bcd9c...`, "create a workflow for managing WhatsApp") silently completed 100% having called `files.read`/`files.write`/`web.search` — never n8n. After this fix, the new mission's task execution log reads: `"Attempt 1/4 failed: Could not reach the n8n integration service. Failed to fetch"` — confirming the **real** n8n tool was correctly selected and genuinely attempted this time (the failure itself is expected and honest: the audit separately confirmed the configured n8n Cloudflare tunnel is currently DNS-dead — a real external outage, not a bug in this fix). The mission correctly resolved `status: 'failed'` rather than a fabricated `'completed'` — exactly the intended outcome. `npm run typecheck` clean.

## M4-02 — PLACEHOLDER TOOLS MUST SIGNAL FAILURE, NOT FAKE SUCCESS (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-02. Branch: `milestone-4-mission-reliability`. Source: the audit's section 1/5 finding.

**Root cause**: `lib/tools/builtin-tools.ts`'s `placeholderHandler()` (backing `files.read`, `files.write`, `web.search`, and the Google/GitHub stubs — all registered `status: 'beta'`, indistinguishable from a real tool to the planner) returned a plain object (`{ message: '...not yet configured...' }`). `lib/tools/executor.ts`'s `execute()` treats **any non-throwing handler return as `ok: true`** (its only failure path is the `catch` around `await registered.handler(...)`) — there was no way for a placeholder's "success" to ever be told apart from a real one.

**What changed**: `placeholderHandler()` now `throw`s a real `Error` instead of returning a value — the executor's actual, only failure signal. This flows through the same retry/backoff and terminal-fail path a genuine tool error would, with zero special-casing needed anywhere downstream.

**Live verification performed** (not typecheck-only): sent a real chat message ("search online for the latest news about AI agents") that routes to `web.search`. Console log confirms the full real sequence: `Tool executing: web.search` → `Tool called for unconfigured service: web.search` → **`error: Tool failed: web.search {error: "web.search is not yet implemented — no real action was taken...", retries: 1, durationMs: 2}`** — a genuine failure event, not a silent success. The system correctly fell through to a real LLM response afterward instead of trusting a fake tool result. `npm run typecheck` clean.

## M4-03 — SURFACE LESSONS_LEARNED PARTIAL-FAILURE SIGNAL IN THE MISSION UI (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-03. Branch: `milestone-4-mission-reliability`. Source: the audit's section 1 finding.

**Gap**: `lib/swarm/missionEngine.ts`'s `recalculateProgress()` already writes a real `lessons_learned` row (`outcome: 'partial'` or `'failure'`) whenever a mission finishes with some tasks failed — the one honest signal in the system — but grep-confirmed no UI component ever read it back.

**What changed**:
- `lib/swarm/types.ts` — new `Lesson` type mirroring `TimelineEntry`'s pattern.
- `lib/swarm/missionService.ts` — new `getLessonsForMission(missionId)` (the read side `recordLesson()` never had); wired into `getFullMission()` (used by `GET /api/missions/[id]`, already tenant-checked there) so it ships for free with the existing mission-detail fetch.
- `app/missions/[id]/page.tsx` — two new banners, distinct from M3-08's "stalled" one (that's mid-flight; this is finished-but-not-clean): an orange **"Completed with failures"** banner when a `partial` lesson exists, and a red **"Mission failed"** banner when a `failure` lesson exists for a `'failed'` mission, both showing the lesson's real `summary`/`detail` text.

**Live verification performed** (not typecheck-only): loaded the real mission `1f3543b5...` (3 of 6 tasks genuinely failed, mission status `'completed'`) — the orange banner renders with the exact real summary ("completed with 3 of 6 tasks failing") and detail (the real per-task error messages). Loaded the real mission created during M4-01's live verification (`e3db3bc9...`, correctly resolved `'failed'`) — the red banner renders with "1 of 1 tasks failed" and the real n8n-unreachable error. `npm run typecheck` clean.

## M4-04 — TIMEOUT GUARD ON VOICEPLAYER.SPEAK() (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-04. Branch: `milestone-4-mission-reliability`. Source: the audit's section 2 finding.

**Root cause**: `lib/voice/voice-player.ts`'s `speak()` only resolved via the browser firing `onend`/`onerror` on `SpeechSynthesisUtterance` — a well-documented real Web Speech API flakiness where neither callback ever fires. With no timeout anywhere in the chain, `voice-manager.ts`'s `speak()` promise (which wraps `VoicePlayer.speak()`) would hang forever, `isProcessingVoice` would stay `true` permanently, and `startListening()`'s guard (`if (this.isProcessingVoice) return;`) would silently swallow every subsequent voice attempt with zero error shown.

**What changed**:
- `lib/voice/voice-player.ts` — added `speakTimeoutFor(text)` (scaled to text length, floored at `MIN_SPEAK_TIMEOUT_MS = 8_000`, capped at `MAX_SPEAK_TIMEOUT_MS = 45_000`) and a `watchdog` timer armed right after `this.synth.speak(u)`. If neither `onend` nor `onerror` fires before the timeout, the watchdog calls `this.stop()` and fires `opts.onError?.()` + `opts.onEnd?.()` itself, guarded by `if (this.utterance !== u) return;` so a stale watchdog from a superseded utterance can never cancel a newer one. `stop()` also clears any pending watchdog.
- `lib/voice/voice-manager.ts` — `speak()`'s `onError` callback previously did a silent `resolve()`; it now calls the existing M3-10 mechanism, `store.setError("Voice playback didn't complete — try again.")`, before resolving — so a real browser `onerror` and the new watchdog timeout both surface a visible error the same way.

**Live verification performed** (not typecheck-only): the first verification attempt (a real end-to-end voice → chat → TTS run) was inconclusive — the real OpenRouter provider returned live `429`/`402` errors (rate-limited, then out of credits) before any reply text existed to speak, so the watchdog was never exercised; that's a separate, unrelated infra problem, not a defect in this fix. Re-ran with the Supabase `ai-chat` edge-function call intercepted (Playwright `page.route`) to return a fixed instant reply, isolating the real `VoicePlayer`/`VoiceManager`/`voiceStore` code path from that external flakiness, combined with a mocked `window.speechSynthesis.speak` that captures the utterance and deliberately never calls `onend`/`onerror` (the exact flakiness this ticket targets). Confirmed: `speak()` called once, `onend`/`onerror` never fired, and ~8s later (past `MIN_SPEAK_TIMEOUT_MS`) the watchdog fired — `synth.cancel()` invoked exactly once and a real, visible red banner **"Voice playback didn't complete — try again."** rendered in the chat UI (screenshot-verified at the exact recovery mark). A follow-up mic click immediately afterward routed normally again (`Routed to Temo` logged, mic listening state restored) — confirming the app recovers rather than staying permanently wedged. `npm run typecheck` clean.

## M4-05 — RE-VALIDATE N8N CONNECTION STATUS INSTEAD OF TRUSTING STALE CACHE (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-05. Branch: `milestone-4-mission-reliability`. Source: the audit's section 1/4 finding.

**Investigation before changing anything**: grep-confirmed `app_settings.n8n_connection_status` (written by `supabase/functions/n8n-proxy/credentialService.ts`'s `saveConnectionStatus()`, called from the proxy's `test-connection` action) has **zero UI readers anywhere** — the stale `"connected":true` the audit found in the raw DB row was never actually displayed to a user as-is. The real gap was narrower than the field itself: `app/settings/page.tsx`'s `N8nSection` already had a real, live "Test Connection" button (calls `services/n8n`'s `testConnection()`, which proxies a genuine `GET /workflows` against the configured n8n URL) — but it only ran on a manual click. Opening Settings → n8n showed a blank, opinion-free form with no indication the configured endpoint was unreachable until the user thought to click Test.

**What changed** (`app/settings/page.tsx`'s `N8nSection`): the section's mount effect now calls the same real `testConnection()` automatically right after loading settings, whenever a `n8n_url` is configured — no manual click required to see accurate live status. Deliberately does **not** call `saveSettings()` first (unlike the manual button, which persists pending edits before testing) — an auto-check on page view shouldn't silently write whatever's currently in the form.

**Live verification performed** (not typecheck-only): loaded Settings → n8n with the real, currently-configured (and still DNS-dead) Cloudflare tunnel URL. The "Connection Failed" banner rendered automatically within seconds of the page loading, with no click, showing the real error: `error sending request for url (.../api/v1/workflows?limit=1): client error (Connect): dns error: failed to lookup address information: Name or service not known`. `npm run typecheck` clean.

## M4-06 — MAKE "LIVE" DASHBOARD WIDGETS ACTUALLY LIVE + FIX THE EMPTY CATCH IN loadMissions() (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-06. Branch: `milestone-4-mission-reliability`. Source: the audit's section 1/5/7 findings.

**Root cause (liveness)**: every "live" widget on Main Dashboard (`components/temo/command-widgets.tsx`'s 9 widgets, plus `command-deck.tsx`'s Mission Control widget) fetched exactly once on mount with zero polling/revalidation — the confirmed cause of "needs a manual refresh every minute for things to work," and independently the reason Mission Control and `/missions` could silently disagree until a full reload.

**Root cause (silent failure)**: `stores/dashboardStore.ts`'s `loadMissions()` had an empty `catch {}` — a query failure left stale/empty state with zero user-facing signal. Investigating this ticket surfaced a **deeper instance of the same problem one layer down**: `lib/swarm/missionService.ts`'s `listMissions()` itself already swallows query errors and returns `[]` on failure (by design, for its other 6 callers — `dashboardService.ts`, `healthService.ts`, `use-gbrain-data.ts` — which intentionally treat "no missions" and "query failed" the same way). That meant even a correct `try/catch` around `listMissions()` in the store could never actually observe a real failure — the promise always resolves "successfully" with an empty array.

**What changed**:
- `components/temo/command-widgets.tsx` — added a shared `usePolled(fetcher, intervalMs)` hook (mirrors `right-sidebar.tsx`'s existing `setInterval` pattern) and switched all 9 widgets (Active Tasks, Runtime Summary, Live Activity, System Health, API Calls, Memory, Knowledge, Analytics, Workflows) from a one-shot `useEffect` to it. Default interval 15s; Analytics uses 60s since it still rides `getExecutionStats()`'s sequential per-mission fetch (M4-07 will parallelize that — a shorter interval would repeatedly hammer a query that's already the audit's confirmed N+1 hotspot).
- `components/temo/command-deck.tsx` — Mission Control's `Promise.all([loadAgents(), listMissions(10)])` fetch now polls every 15s the same way, and gained a `.catch()` (previously none at all — a rejected promise left `data` null forever, stuck on the "Initializing..." screen).
- `lib/swarm/missionService.ts` — added `listMissionsOrThrow()`, a sibling to `listMissions()` with the identical query but that actually throws on a real Postgres/network error, specifically for the one caller that needs to tell "empty" apart from "failed." `listMissions()` itself is untouched — its silent-`[]` contract stays intact for its existing callers, avoiding a wider regression across dashboard stats/health/G-Brain.
- `stores/dashboardStore.ts` — new `missionsError: string | null` state; `loadMissions()` now calls `listMissionsOrThrow()` and sets `missionsError` to the real message on failure (clearing it on success), instead of the empty catch.
- `app/missions/page.tsx` — reads `missionsError` and renders a real error banner when set; also now polls `loadMissions()` every 15s instead of fetching once on mount.

**Live verification performed** (not typecheck-only): confirmed via real network-request counts that both Mission Control (dashboard) and `/missions` refetch on their own — `missions` REST calls went from 2 to 11 and `mission_tasks` calls from 0 to 74 within 16s of page load with no user action, proving the poll intervals actually fire. Separately, intercepted the real `/rest/v1/missions` request to return a 500 and confirmed a real, visible red banner — "Failed to load missions: simulated failure" (the real Postgrest error message, not a canned string) — renders on `/missions`, where before this fix the empty catch (two layers deep) would have silently shown "No Active Missions," indistinguishable from a genuinely empty account. `npm run typecheck` clean; `npm run lint` shows only pre-existing warnings unrelated to these files.

## M4-07 — PARALLELIZE getExecutionStats()'S PER-MISSION TASK FETCH (2026-08-27)

**Ticket**: `docs/BACKLOG-M4.md` M4-07. Branch: `milestone-4-mission-reliability`. Source: the audit's section 7 finding.

**Root cause**: `lib/dashboard/dashboardService.ts`'s `getExecutionStats()` fetched up to 200 missions, then looped `await getTasks(m.id)` **sequentially** — a real, worsening N+1 that ran on every Main Dashboard mount via `AnalyticsWidget` (and every 60s now, per M4-06's polling). `getSystemStats()` had the identical pattern.

**What changed**: both functions now issue their per-mission `getTasks()` calls via `Promise.all(missions.map(...))` instead of an `await`-in-a-loop — same data, same shape, fetched concurrently instead of one-at-a-time. `getSystemStats()` is fixed too even though it's currently dead in practice (only reachable via the orphaned `/api/stats/dashboard` route, per the audit) — not left as a landmine for whenever that route gets wired up.
...
**What changed**: `app/missions/[id]/page.tsx` (Objectives/Timeline/Tasks) and `components/temo/org-chart.tsx` (G-Brain active-mission indicator) now subscribe to Supabase Realtime (`postgres_changes`) on their underlying tables via the new `lib/hooks/useRealtimeRefetch.ts`, instead of relying solely on a fixed-interval poll (M7-06's 3s, M6-05's 5s). A slow 30s safety-net poll remains alongside the subscription in both places — deliberate, not an oversight — as a reconciliation fallback for a dropped Realtime connection.

**Required migration**: `supabase/migrations/20260903120000_enable_realtime_mission_tables.sql` adds `missions`/`mission_tasks`/`mission_objectives`/`mission_timeline` to the `supabase_realtime` publication — Realtime only delivers events for tables explicitly published, and no table in this project had ever been added before this. Additive-only, idempotent (`IF NOT EXISTS` guards). Dry-run verified via a real `BEGIN`/`ROLLBACK` against the live DB (Supabase CLI `db query --linked`) before being applied for real, matching the M5-13 migration-review standard.

**SECURITY NOTE — RLS enforcement for Realtime on these 4 tables (reviewed and approved by Cowork + Amro before the migration was applied)**: Supabase Realtime's `postgres_changes` automatically enforces each table's existing RLS SELECT policy per subscriber for INSERT/UPDATE events (confirmed against Supabase's own docs: "filtered-out events never leave the database"), and this project's Realtime server is confirmed live-patched as recently as 2026-07-09 (`realtime.schema_migrations`), well past the 2023-era introduction of this behavior. It does **not** enforce RLS for DELETE events — a documented Supabase limitation (no way to check access to an already-deleted row). Confirmed this is currently a dormant gap only: none of the 4 tables has an authenticated-role DELETE RLS policy (`20260819140000_create_v1_corporate_os_foundation.sql`, `20260820140000_fix_missions_tenant_leak.sql`), so no browser session can ever cause a DELETE on them in the first place — RLS rejects it outright before any row is removed. The only DELETE calls anywhere in the codebase are `tests/mission-lifecycle.test.ts`'s own service-role cleanup of its own test data, never reachable from the running app. **Constraint for future work, recorded here and inline in the migration**: do not add a client-facing hard delete on any of these 4 tables without re-reviewing this Realtime/RLS interaction — prefer the existing soft-delete pattern (e.g. `missions.status = 'cancelled'`) if a delete-like capability is ever needed.
...
Version: 3.25
Date: 2026-08-27
Status: Milestone 1 and Milestone 2 complete and merged to `main`. Milestone 3 (Speed & Stability + Visual/UX fixes): M3-01/02/04/05 merged to `main`. Completion pass on `milestone-3b-voice-fixes`: **all of M3-07 through M3-11, M3-06, and M3-03 are now done and live-verified — Milestone 3 is fully implemented.** M3-07: command-deck.tsx now populates the shared agent store and initializes `CrewCoordinator`'s registry, fixing "No agent selected" when Main Dashboard is the first page visited in a session. M3-08: root-caused the 20%-stalled mission to the client-side mission-execution model — a real DB-level recovery function exists but is dormant without a deployed `pg_cron` target; the mission detail page now surfaces an honest "stalled" state instead of silence. M3-09: removed the old icon-only VoiceHud popover everywhere — voice is now exactly Main Dashboard's `VoiceTrigger` and Chat's own mic. M3-10: extracted `VoiceTrigger` into a shared component and added a real visible voice-error state. M3-11: fixed the fabricated "Gemini Live API" label and a genuine bug where every agent's placeholder voice config silently overrode the user's actual Settings selection. M3-06: added anti-filler persona guidelines and fixed the Arabic directive to Egyptian colloquial dialect. M3-03: replaced the tree-based G-Brain layout with a real concentric-ring radial layout (Temo center, Corporate Office ring, Operating Company ring, worker ring, size decreasing by depth), added a hover-card, fixed a real pointer-events bug in the voice aura, and tightened Main Dashboard's mini-preview grid. Predecessor milestones: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
