...
Version: 3.3
Date: 2026-08-26
Status: M1-01 through M1-06 (Milestone 1 — Reliability & Safety) implemented and live-verified. M1-01: the tool executor is wired into the mission engine's `executeTask()`. M1-02: `checkBudget()` is a real hard gate inside `chatWithFallback()`/`streamWithFallback()`. M1-03: a Postgres token-bucket rate-limits the 2 API routes that can trigger an AI call or tool execution. M1-04: n8n's `handleRun()` now polls real execution history for a genuine terminal status instead of trusting the trigger's HTTP code. M1-05: the chat page resumes the tenant's most recent conversation on load. M1-06: audited all 23 `app/api/**` routes — no missing auth found, but 2 real IDOR gaps fixed (`stream/mission`, `tasks/queue` now verify tenant ownership of an attacker-controlled `missionId`, matching the pattern already proven for `missions/[id]/timeline`); 16 routes remain authenticated-but-globally-scoped, flagged as an explicit open product decision rather than guessed at. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
