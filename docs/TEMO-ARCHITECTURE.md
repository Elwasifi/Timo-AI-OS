...
Version: 2.9
Date: 2026-08-26
Status: M1-01 and M1-02 (Milestone 1 — Reliability & Safety) implemented and live-verified. M1-01: the tool executor is now wired into the mission engine's `executeTask()`, reusing the chat path's `decideTools()`; a client-side-only tool-registry population bug was fixed (server-executed mission tasks would have silently never run any tool); once local n8n + `cloudflared` became available, three real bugs in `supabase/functions/n8n-proxy` were found and fixed (missing required `settings` on workflow creation, wrong response-unwrapping on every single-resource n8n endpoint, `convertToWebhook` never activating the workflow it created), then re-verified with a real n8n execution confirmed via n8n's own execution history. M1-02: `checkBudget()` is now a real hard gate inside `chatWithFallback()`/`streamWithFallback()` — the single choke point every real AI call passes through regardless of caller — with Ollama exempt (zero marginal cost) and the internal tenant seeded with a real, adjustable ceiling instead of being unlimited by default; live-verified with a real over-budget tenant (hard block with a clear message), the Ollama exemption, and a no-tenant regression check. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback`, deciding provider+model per request from task classification, real provider-reported capability/pricing data, and live health tracking — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
