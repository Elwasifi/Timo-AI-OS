...
Version: 3.10
Date: 2026-08-26
Status: Milestone 1 and Milestone 2 complete and merged to `main`. Milestone 3 (Speed & Stability + Visual/UX fixes) underway on `milestone-3-experience`: **M3-01 done and live-verified** — fixed two dead/misconfigured provider models (NVIDIA EOL, OpenRouter no-credits) and the real silent-non-response bug (reasoning models returning empty content on a 200 success when `maxTokens` is exhausted by internal reasoning), added a health-based skip-before-retry so a recently-failing provider isn't retried 3x before fallback, and bounded the intent classifier's hot-path exposure with a hard timeout. M3-02 (fast-first-response guarantee), M3-04 (Command Deck → Main Dashboard rename + G-Brain link), and M3-05 (voice trigger redesign + chat-page mic bug fix) remain in progress. M3-03 (G-Brain radial layout redesign) and M3-06 (Timo persona/system-prompt pass) are explicitly out of scope for this milestone. Predecessor milestones: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
