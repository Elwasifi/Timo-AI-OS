...
Version: 3.9
Date: 2026-08-26
Status: Milestone 1 complete and merged to `main`. Milestone 2 (Beta Readiness) underway on `milestone-2-beta-readiness`: **M2-01 and M2-02 both done and live-verified.** M2-01: all 16 routes M1-06 left undecided now have a definitive resolution (8 real tenant-filtering fixes, 8 documented as permanently global-by-design). M2-02: the `ai-chat` edge function — the one real, unbypassable choke point every AI provider call passes through — now enforces a per-IP rate limit unconditionally; per-tenant granularity was considered and deliberately deferred (would require threading tenant identity through 3 internal AI call sites that don't carry it today), flagged as an open follow-up rather than built speculatively. M2-03 (named Cloudflare tunnel cutover) remains an owner action pending Amro's `cloudflared tunnel login`. Predecessor milestone: the Dynamic Model Router (`lib/ai/router/`, 2026-08-20) sits between every real AI call site and `chatWithFallback`/`streamWithFallback` — see the "DYNAMIC MODEL ROUTER" section above for full detail. Full prior-version history remains in the dated sections above this footer, in order.
